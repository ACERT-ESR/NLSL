c  VERSION 1.0  (NLSMC version)   2/5/99
c----------------------------------------------------------------------
c
c                         ====================
c                          subroutine MATRXO
...
c
c    This routine calculates the matrix elements off-diagonal subspace
c (pS=+/- 1, qS=0).  Nonsecular terms in the Hamiltonian is ignored
...
c pulse propagator.
c
c    M-symmetrization is included.  In the absence of nuclear Zeeman,
c the matrix becomes block-diagonal form and we need to keep only the
c block with jM=1 since the starting vector has non-zero elements in
c jM=1 and pulse propagator does not mix jM=1 and jM=-1 block.  In the
c presence of nuclear Zeeman, the block with jM=-1 is included in the
c calculation.
c
...
c    T2ed, T2ef for the electronic relaxation.
c
c      T2ed : orientationally invariant electronic T2 relaxation
c             This defines the homogeneous line-width of ESR transition.
c      T2ef : angular variation of T2e values.  This angular variation
c             is naturally defined by the angle between B0 and the
c             molecular z-axis.  It is defined in the angle between
c             director and the molecular z-axis for convenience initially.
c             And then it is transformed to B0 is necessary.
c
c   *Note that the sign of these relaxation constants are reversed
c    with respect to the time domain formalism of the relaxation processes     
...
c
c         Rotationally invariant line width : T2 electronic relaxation
c
          if(iper.ne.0) cdfd=cdfd+t2ed
c
          cdfd1=cdfd
c
        end if
...
c
c           Angular variation of the line width : T2 electronic relaxation
c
...
c
     ...
     #        .and.(jkd.eq.0).and.((kdabs.le.kband).or.
     #        (ksabs.le.kband))) then
c
              kdi=1+kdabs/2
              ksi=1+ksabs/2
...
c
              if((jmd.eq.0).and.(in2.ne.0)) then
c                                         *** difference term ***
                clioua1=zero
                cliou1=zero
                if((ipndab.eq.iqndab).and.(ipndab.le.1).and.(mdabs.
     #            le.2))  then
c
                  cga0=zero
                  cga2=zero
...
c                                         *** sum term ***
                clioua2=zero
                cliou2=zero
                if((ipnsab.eq.iqndab).and.(ipnsab.le.1).and.(msabs.
     #            le.2))  then
c
                  cga0=zero
                  cga2=zero
...
c					      (delta pS,pI .eq. 0)
	            ...
	            cga2=dsq23
                  ...
                  if(flk0.and.(jkd.eq.0).and.(ms.eq.0))
     #                                      cliou2=sa1*cga0*a0
                end if
c
...
c
c        **   Electronic Zeeman interaction   **
c
              cliou1=zero
              cliou2=zero
              clioug1=zero
              clioug2=zero
              if((jmd.eq.0).and.(iqnd.eq.0)) then
c
                if(flk0.and.(jkd.eq.0).and.(md.eq.0).and.
     #             (ipnd.eq.0))            cliou1=iper*b0
c
                if((ipnd.eq.0).and.(abs(rg).gt.rndoff)) then
                  sg1=iper*dsq23
                  clioug1=sg1*d1*rg
                end if
c
                if(flk0.and.(jkd.eq.0).and.(ms.eq.0).and.
     #             (ipns.eq.0))            cliou2=iper*b0
c
                if((ipns.eq.0).and.(abs(rg).gt.rndoff)) then
                  sg1=iper*dsq23
                  clioug2=sg1*d2*rg
                end if
c
...
c
              cliou1=zero
              cliou2=zero
              if(fnz.and.(jmd.ne.0).and.flk0.and.(jkd.eq.0).
     #                           and.(iqnd.eq.0)) then
                if((ipnd.eq.0).and.(md.eq.0)) cliou1=ipnr*zeen
                if((ipns.eq.0).and.(ms.eq.0)) cliou2=ipnr*zeen
...
c
            if((dabs(ossc).gt.rndoff).and.(ipnd.eq.0).and.flk0
     #         .and.(md.eq.0).and.(jkd.eq.0).and.(jmd.eq.0)) then
              c=zero
              if(iqnd.eq.0) c=one
...
c
     ...
     #        (jkd.eq.0).and.(jmd.eq.0)) then
              cgam=cgam+cdff
              if(fld.and.(kd.eq.0))   cgam=cgam+ct2efi
            end if
c
          end if
c
